...
def fallback_post(target_date: str, recent_ids: list[str], model_name: str) -> dict[str, Any]:
    choices = ["horizon-chart", "upset", "marimekko", "parallel-sets", "streamgraph"]
    chart_id = next((item for item in choices if item not in recent_ids), choices[0])
    ...
    return {
        ...
        "novelty_score": score,
        "repeat_risk": "medium" if chart_id in recent_ids else "low",
...
def main() -> int:
    ...
    if gemini_key:
        prompt = build_prompt(
            ...
            chart_examples=list(config.get("chart_candidates", [])),
        )
        ...
        payload = fallback_post(target_date, recent_ids, "fallback-no-api-key")
